...
let part1 content =
 let map' = parse content in
 match bfs map' with
 | None -> raise @@ Exception "HELP CANNOT FIND A WAY"
 | Some d -> d
...
let solve filename =
  ...
  let content = String.split_lines content in
  printf "fewest required steps: %d\n" @@ part1 content;
  printf "really fewest steps required: %d\n" @@ part2 content;;
