...
package Sensor 
{
	...
	sub BUILD ($self, $arg)
    {
		my $d = $self->distance($arg->{closest_beacon});
	...
	sub border_points ($self)
    {
        ...
        {
			my $step = $_;
			Point::->new(x => $self_x + $step, y => $self_y + $step - $d), 
			Point::->new(x => $self_x + $d - $step, y => $self_y + $step), 
			Point::->new(x => $self_x - $step, y => $self_y + $d - $step), 
			Point::->new(x => $self_x + $step - $d, y => $self_y - $step), 
		} 1 .. $d;
	}
